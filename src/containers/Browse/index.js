...
import { updateFetchedBooks } from "./actions";
...
class Browse extends Component {
  ...
  fetchBooks = () => {
    ...
    //where('loanData','==',null).orderBy('title','asc').get() for only borrowed Books
    booksRef.get().then(querySnapshot => {
      ...
      this.props.updateFetchedBooks(books);
  ...
  render() {
    const books = this.props.browse.filteredBooks;
    ...
    return (
      <div className="listContainer">
        {this.state.isLoading ? (
          ...
          !!books &&
          books.map((book, i) => <BookInfoCard book={book} key={i} />)
        )}
      </div>
    );
  }
...
export default connect(
  ({ browse }) => ({ browse }),
  dispatch => bindActionCreators({ updateFetchedBooks }, dispatch)
)(Browse);
